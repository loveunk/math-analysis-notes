...
**定义2.1.3**：设$f: A \rightarrow B$，如果$f(a)=B$，则称$f$是从$A$到$B$上的**满射**，也就是说，$B$中的任何元素都是$A$中某一元素在$f$之下的像。

**定义2.1.4**：设$f: A \rightarrow B$，如果当$x, y \in A$，且$x \neq y$时，有$f(x) \neq f(y)$，则称$f$为**单射**。

**定义2.1.5**：设$f: A \rightarrow B$，既是单射又是满射，则称映射$f$是**一对一**的，这时，也说$f$在集合$A$与$B$之间建立一个**一一对应**。
